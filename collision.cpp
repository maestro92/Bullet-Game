...
bool collision::raysphere1(vector3d c, vector3d rd,vector3d s,float r,float* dist,vector3d* collpoint)
{
	float b=2*(rd.x*(s.x-c.x)+rd.y*(s.y-c.y)+rd.z*(s.z-c.z));

	float ce=s.x*s.x-2*s.x*c.x+c.x*c.x+
            s.y*s.y-2*s.y*c.y+c.y*c.y+
            s.z*s.z-2*s.z*c.z+c.z*c.z-r*r;
	...
	if(disc<0)	//if the discriminant is less then 0, there is no intersection
		...
		if(dist!=NULL)
		{
			...
			if(collpoint!=NULL)
			{
				float x=s.x+(*dist)*rd.x;
				float y=s.y+(*dist)*rd.y;
				float z=s.z+(*dist)*rd.z;
				collpoint->change(x,y,z);
			}
		}
		return true;
...
bool collision::sphereplane(vector3d& sp,vector3d pn,vector3d p1,vector3d p2,vector3d p3,vector3d p4, float r, bool up)
{

...
//	cout << path << endl;
	...
	for(int i=0;i<coord.size();i++)
	{
		...
		}else if((*coord[i])[0]=='f')
		{
			...
			if(coll && collplane!=NULL)
			{
				...
				collplane->push_back(collisionplane(normals[b-1]->x,normals[b-1]->y,normals[b-1]->z,vertex[a-1]->x,vertex[a-1]->y,vertex[a-1]->z,vertex[c-1]->x,vertex[c-1]->y,vertex[c-1]->z,vertex[d-1]->x,vertex[d-1]->y,vertex[d-1]->z,vertex[e-1]->x,vertex[e-1]->y,vertex[e-1]->z, materialId));
			}else
			{
	...
	}else if((*coord[i])[0]=='u' && (*coord[i])[1]=='s' && (*coord[i])[2]=='e')
	{
		...
		if(strncmp(tmp,"collision",9)==0)
		{
            ...
            int id=0;
            int k=9;
            while(k < strlen(tmp) && tmp[k] >= '0' && tmp[k]<='9')
            {
                id *= 10;
                id += (tmp[k] - '0');
                k++;
...
unsigned int objloader::loadTexture(const char* filename)
{
    ...
    //	SDL_Surface* img=SDL_LoadBMP(filename);
        ...
        textureNames.push_back(string(filename));
	return num;
}

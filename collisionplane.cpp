...
collisionplane::collisionplane(float n1, float n2, float n3, float a,float b, float c,    float d, float e, float f,   float g, float h, float i,   float j, float k, float l, int id)
{
	p[0].change(a,b,c);
	p[1].change(d,e,f);
	p[2].change(g,h,i);
...
bool collisionplane::Valid_Nomral()
{
